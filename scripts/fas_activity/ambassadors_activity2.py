...
"""
This program retrieves all the people in the ambassador group use FAS and
...
def get_fas_userlist(fas_credentials):
    ...
    for people in request['people']:
        if 'group_roles' in people and \
                'ambassadors' in people['group_roles'] and \
                people['group_roles'][
                    'ambassadors']['role_status'] == 'approved':
            output.append(
...
def main():
    ...
    print '%s ambassadors to investigate' % len(peoples)
    if not peoples:
        return
    ...
    with open('amba_active_date2.csv', 'w') as stream:
    ...
    print 'amba_active_date2.csv written'

    with open('amba_active_per_day2.csv', 'w') as stream:
        stream.write('Days,Numbers\n')
        days = sorted(output.keys())
        for key in days:
            stream.write('{0},{1}\n'.format(key, output[key]))
    print 'amba_active_per_day2.csv written'
